...
val prog = ``BirProgram
		 [<|bb_label :=
		  ...
		  bb_last_statement := BStmt_Jmp (BLE_Label (BL_Address (Imm32 2804w)))|>;
						 <|bb_label :=
		  BL_Address_HC (Imm32 2804w) "11000400 (add w0, w0, #0x1)";
		  bb_statements :=
		  [BStmt_Assign (BVar "R0" (BType_Imm Bit32))
		  ...
		  BStmt_CJmp (BExp_Den (BVar "ProcState_Z" BType_Bool))
			     ...
			     <|bb_label := BL_Address_HC (Imm32 2836w) "110008A5 (add w5, w5, #0x2)";
		  bb_statements :=
		  [BStmt_Assign (BVar "R5" (BType_Imm Bit32))
				(BExp_Cast BIExp_UnsignedCast
					   (BExp_BinExp BIExp_Plus
							(BExp_Cast BIExp_LowCast
								   (BExp_Den (BVar "R5" (BType_Imm Bit32))) Bit32)
							(BExp_Const (Imm32 2w))) Bit32)];
		  bb_last_statement := BStmt_Jmp (BLE_Label (BL_Address (Imm32 2840w)))|>;
						 <|bb_label :=
		  BL_Address_HC (Imm32 2840w) "14000002 (b 30 <.text+0x30>)";
		  bb_statements := [];
		  bb_last_statement := BStmt_Jmp (BLE_Label (BL_Address (Imm32 2848w)))|>;
						 <|bb_label := BL_Address_HC (Imm32 2844w) "110004A5 (add w5, w5, #0x1)";
		  bb_statements :=
		  [BStmt_Assign (BVar "R5" (BType_Imm Bit32))
				(BExp_Cast BIExp_UnsignedCast
					   (BExp_BinExp BIExp_Plus
							...
							(BExp_Const (Imm32 1w))) Bit32)];
		  bb_last_statement := BStmt_Jmp (BLE_Label (BL_Address (Imm32 2848w)))|>;
						 <|bb_label := BL_Address (Imm32 2848w); bb_statements := [];
...
val stop_lbl_tms = [“BL_Address (Imm32 2848w)”];
